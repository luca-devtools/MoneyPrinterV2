"""
Vendored copy of ``src/lib/youtube_search.py``.

The web app deploys as an isolated unit (its own light ``requirements.txt``, no
access to the CLI's ``src/`` tree), so the pure search+parse logic is copied
here verbatim. Keep this in sync with ``src/lib/youtube_search.py`` when the
parsing logic changes — the two are intentionally identical.
...
# "EgIQAQ==" restricts results to the "Video" type.
...
def read_text(node) -> str:
    """Reads a YouTube ``{"simpleText": ...}`` or ``{"runs": [...]}`` node."""
